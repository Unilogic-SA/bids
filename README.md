...
3. The listing page reads the latest successful and failed syncs from Supabase. It shows a critical warning when no successful refresh exists or the latest success is older than 26 hours. A newer failure with otherwise fresh data is shown as a less severe delay while the automatic schedule retries.
